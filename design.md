...
3. **Real-time Awareness**: Live store telemetry (occupancy count, cooler temperature, empty shelf warnings) visible without navigating deeply into menus.
...
| `warning` | `#FFB020` | Low stock warnings, near-expiry items, threshold alerts |
| `danger` | `#FF6B6B` | Critical alerts, expired items, empty shelves, delete buttons |
| `purple` | `#7B61FF` | AI features (Gemini chatbot, ML forecast badges) |
...
└── (tabs)/                     # Bottom navigation tabs
    ...
    ├── index.tsx               # Home Dashboard (Retail KPIs + IoT Telemetry)
...
## 4. New Edge & IoT UI Components Designed

To interface with the newly created backend monitoring and telemetry services, the following UI modules and components are specified:
...
- **Visuals**:
  ...
  - State indicator:
    ...
    - Empty / Refill Now (Red, fill < 20%)

### C. Live IoT Telemetry Carousel
...
### D. System Alerts Center / Floating Banner
...
The frontend [`frontend/services/api.ts`](file:///c:/SIH2026/frontend/services/api.ts) file will export the following typed functions to consume the new backend endpoints:
...
| **v2.1** | **Home Dashboard Telemetry** | Integrate `fetchDashboardOverview` in `index.tsx`. Add Live Occupancy counter, IoT Device status chip, and Open Alerts badge. | `IN PROGRESS` |
| **v2.2** | **Smart Shelf Visualizer** | Add Visual Shelf fill-percentage cards on Home / Inventory screen with out-of-stock restock buttons. | `PLANNED` |
| **v2.3** | **Alerts Notification Center** | Slide-up modal or top bar dropdown showing real-time critical alerts with one-tap "Mark Resolved". | `PLANNED` |
| **v2.4** | **Camera Surveillance & Footfall Heatmap** | Add snapshot viewer for entrance camera events and hourly footfall graph (entries vs. exits). | `PLANNED` |
...
- **2026-09-09**:
  - Created master `design.md` covering typography, design tokens, screen architecture, new IoT component specifications, and roadmap checkpoints.
  - Linked design specifications with newly implemented backend monitoring and telemetry endpoints.
